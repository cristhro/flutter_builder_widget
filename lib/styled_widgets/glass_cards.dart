...
import 'package:builder_widgets/_widgets/gradient_container.dart';
...
class CollapsingCard extends StatefulWidget {
  const CollapsingCard({Key? key, required this.child, required this.height, required this.title, this.icon, this.titleClosed}) : super(key: key);
  final Widget child;
  final double height;
  final String title;
...
class _CollapsingCardState extends State<CollapsingCard> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    double headerHeight = 40; //context.select((AppModel m) => m.enableTouchMode) ? 50 : 40;
    return SizedBox(
...
class _CollapsableCardHeader extends StatelessWidget {
  const _CollapsableCardHeader({Key? key, required this.onPressed, required this.height, required this.animation, required this.isOpen, required this.title, this.icon}) : super(key: key);
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  HSpace.xs,
                  Expanded(child: Text(title.toUpperCase(), maxLines: 1, style: TextStyles.callout2.copyWith(color: theme.greyStrong))),
                  if (icon != null) icon!,
                ],
              ),
            )),
