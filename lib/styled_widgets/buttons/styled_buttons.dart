...
class PrimaryBtn extends StatelessWidget {
  const PrimaryBtn({Key? key, required this.onPressed, this.label, this.icon, this.child, this.leadingIcon = false, this.isCompact = false, this.cornerRadius}) : super(key: key);
  final Widget? child;
  final String? label;
  final IconData? icon;
...
class SimpleBtn extends StatelessWidget {
  const SimpleBtn({Key? key, required this.onPressed, required this.child, this.focusMargin, this.normalColors, this.hoverColors, this.cornerRadius, this.ignoreDensity}) : super(key: key);
  final Widget child;
  final VoidCallback? onPressed;
...
class TextBtn extends StatelessWidget {
  const TextBtn(this.label, {Key? key, required this.onPressed, this.isCompact = false, this.style, this.showUnderline = false}) : super(key: key);
  final String label;
  final VoidCallback? onPressed;
  final bool isCompact;
  ...
  @override
  Widget build(BuildContext context) {
    TextStyle finalStyle = style ?? TextStyles.caption.copyWith(decoration: showUnderline ? TextDecoration.underline : TextDecoration.none, fontWeight: FontWeight.w500);
    return SimpleBtn(
      ignoreDensity: false,
      onPressed: onPressed,
...
class IconBtn extends StatelessWidget {
  const IconBtn(this.icon, {Key? key, required this.onPressed, this.color, this.padding, this.ignoreDensity}) : super(key: key);
  final IconData icon;
  final VoidCallback? onPressed;
  final Color? color;
  ...
  @override
  Widget build(BuildContext context) {
    bool enableTouchMode = false; // context.select((AppModel m) => m.enableTouchMode);
