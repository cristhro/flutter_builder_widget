import 'package:builder_widgets/_utils/string_utils.dart';
import 'package:builder_widgets/_widgets/decorated_container.dart';
import 'package:builder_widgets/_widgets/positioned_all.dart';
import 'package:builder_widgets/_widgets/rounded_card.dart';
...
class _RawBtnState extends State<RawBtn> {
  ...
  Widget build(BuildContext context) {
    MaterialStateProperty<T> getMaterialState<T>({required T normal, required T hover}) => MaterialStateProperty.resolveWith<T>((Set states) {
        ...
        children: [
          ...
          DecoratedContainer(
            ...
            child: TextButton(
              ...
              style: ElevatedButton.styleFrom(
                ...
                side: getMaterialState(normal: BorderSide(color: normalColors.outline ?? Colors.transparent), hover: BorderSide(color: hoverColors.outline ?? Colors.transparent)),
          ...
          if (_focusNode.hasFocus) ...[
            PositionedAll(
              ...
              all: focusMargin,
              child: RoundedBorder(radius: widget.cornerRadius ?? (Corners.med - (focusMargin * .6)), color: theme.focus),
            )
          ],
        ],
      ),
...
class BtnContent extends StatelessWidget {
  const BtnContent({Key? key, this.label, this.icon, this.child, this.leadingIcon = false, this.isCompact = false, this.labelStyle}) : super(key: key);
